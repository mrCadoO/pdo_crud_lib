...
class Database{
	...
	const db_name = "test_pdo";//The Database name
	const user = "root";//The user name used to connect to the Database
	const pass = "loveis";//The password used to connect to the Database
    ...
    public function FetchOneClass($query, $params=NULL, $fetch_mode=PDO::FETCH_CLASS){
        $result = NULL;
        try{
            $this->Execute($query, $params);
            $result = $this->currentStatement->fetchObject(__CLASS__);
        }//try
        catch(PDOException $ex){
            echo "PDO FetchOne Error : ".$ex->getMessage();
        }//catch
    ...
    public function FetchAllClass($query,$params=NULL,$fetch_mode=PDO::FETCH_CLASS){
        $result = NULL;
        try{
            $this->Execute($query, $params);
            $result = $this->currentStatement->fetchAll($fetch_mode);
	...
	public function Execute($query, $param){
		try{
			...
			$this->currentStatement->execute($param);
		}//try
		catch(PDOException $ex){
	...
	// Add attributes to execute like ':attribute' => 'attribute'
	public function AddAttributes($field_name, $field_value){
        $arr = [];
        if(is_array($field_name)){
            for($i = 0; $i < count($field_name); $i++) {
                $arr[':'.$field_name[$i]] = $field_value[$i];
            }
        } else {
    ...
    //REFACTOR
	public function RowCountQuick($query=""){
		$db = new Database();
		return $db->Rows();
	}
